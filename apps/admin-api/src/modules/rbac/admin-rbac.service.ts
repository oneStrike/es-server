...
export class AdminRbacService {
  ...
  async syncCodePermissions(definitions: AdminPermissionDefinition[]) {
    ...
    await this.drizzle.withTransaction({
      execute: async (tx) => {
        await this.lockRbacMutationInTransaction(tx)
  ...
  // 串行化超级管理员授予、移除和禁用路径。
  async lockSuperAdminMutationInTransaction(tx: AdminRbacTransaction) {
  ...
  // 串行化所有 RBAC 表的关系写入；后台域低频写入以完整性优先。
  private async lockRbacMutationInTransaction(tx: AdminRbacTransaction) {
    await acquireIntegrityLocks(tx, [
      ADMIN_RBAC_RELATION_INTEGRITY_LOCKS.mutation,
    ])
  }
