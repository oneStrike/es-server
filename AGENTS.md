...
- 计数器、聚合读模型与冗余统计字段规范以 `.trae/rules/COUNTER_SPEC.md` 为准；实现模式与仓库现状对照参考 `.trae/references/COUNTER_PATTERNS.md`、`.trae/references/COUNTER_REGISTRY.md`。
...
- 条件构建统一使用 `SQL[] + and(...)`；无条件时传 `where: undefined`。
...
2. 先查看 `.trae/rules/RULE_INDEX.md`，再读取对应规范：
   ...
   - Counter：`.trae/rules/COUNTER_SPEC.md`，需要实现模式或现状对照时再看 `.trae/references/COUNTER_PATTERNS.md`、`.trae/references/COUNTER_REGISTRY.md`
