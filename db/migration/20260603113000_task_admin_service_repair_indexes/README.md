...
`db:migrate:prod` and `db:migrate` run migrations through Drizzle migrators, so the checked-in SQL uses normal `CREATE INDEX IF NOT EXISTS` statements. For large production tables, create these indexes manually with `CONCURRENTLY` outside the migration transaction first, then apply this migration during a maintenance window so the statements are no-ops.
