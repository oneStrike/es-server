...
COMMENT ON COLUMN "public"."check_in_plan"."status" IS E'计划状态（0=草稿，1=已发布，2=已下线，3=已停用）。';
COMMENT ON COLUMN "public"."check_in_plan"."cycle_type" IS E'周期类型（1=按周切分，2=按月切分）。';
...
COMMENT ON COLUMN "public"."check_in_record"."reward_status" IS E'基础签到奖励状态（0=待处理，1=已成功，2=已失败）。';
COMMENT ON COLUMN "public"."check_in_record"."reward_result_type" IS E'基础签到奖励结果类型（1=本次真实落账，2=命中幂等未重复落账，3=本次处理失败）。';
...
COMMENT ON COLUMN "public"."check_in_streak_reward_grant"."grant_status" IS E'连续奖励发放状态（0=待处理，1=已成功，2=已失败）。';
COMMENT ON COLUMN "public"."check_in_streak_reward_grant"."grant_result_type" IS E'连续奖励发放结果类型（1=本次真实落账，2=命中幂等未重复落账，3=本次处理失败）。';
...
COMMENT ON COLUMN "public"."growth_audit_log"."asset_type" IS E'资产类型（1=积分，2=经验值，3=徽章）';
...
COMMENT ON COLUMN "public"."growth_rule_usage_slot"."asset_type" IS E'资产类型（1=积分，2=经验值）';
COMMENT ON COLUMN "public"."growth_rule_usage_slot"."rule_key" IS E'规则键（如 points:10 / experience:6）';
COMMENT ON COLUMN "public"."growth_rule_usage_slot"."slot_type" IS E'槽位类型（1=每日限额，2=总限额，3=冷却占位）';
...
COMMENT ON COLUMN "public"."task_assignment"."reward_status" IS E'奖励结算状态。0=待结算，1=已结算成功，2=结算失败。';
COMMENT ON COLUMN "public"."task_assignment"."reward_result_type" IS E'奖励结算结果类型。1=本次真实落账，2=命中幂等未重复落账，3=本次结算失败。';
...
COMMENT ON COLUMN "public"."user_experience_rule"."type" IS E'规则类型（取值见成长规则枚举）。\n举报奖励当前以“举报有效 / 举报无效”表达裁决结果，章节类编码统一收敛到 300 / 400 段。';
...
COMMENT ON COLUMN "public"."user_point_rule"."type" IS E'规则类型（取值见成长规则枚举）。\n举报奖励当前以“举报有效 / 举报无效”表达裁决结果，章节类编码统一收敛到 300 / 400 段。';
