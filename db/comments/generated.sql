...
COMMENT ON COLUMN "public"."task_definition"."code" IS E'任务稳定编码。';
...
COMMENT ON COLUMN "public"."task_definition"."priority" IS E'任务优先级。0=默认优先级，数值越大越靠前。';
...
COMMENT ON COLUMN "public"."task_definition"."repeat_timezone" IS E'周期切分使用的 IANA 时区标识。';
COMMENT ON COLUMN "public"."task_definition"."start_at" IS E'生效开始时间。';
COMMENT ON COLUMN "public"."task_definition"."end_at" IS E'生效结束时间。';
COMMENT ON COLUMN "public"."task_definition"."audience_segment_id" IS E'受众分群键；开放值，由上层配置系统约束。';
...
COMMENT ON COLUMN "public"."task_step"."progress_mode" IS E'进度模式。1=一次完成；2=普通累计；3=按唯一对象累计。';
COMMENT ON COLUMN "public"."task_step"."event_code" IS E'目标事件编码；手动步骤为空。';
COMMENT ON COLUMN "public"."task_step"."target_value" IS E'目标值；必须为大于 0 的整数。';
...
COMMENT ON COLUMN "public"."task_step"."unique_dimension_key" IS E'唯一维度键；仅 `unique_count` 步骤使用。';
