...
COMMENT ON COLUMN "public"."app_announcement"."enable_platform" IS E'启用的平台列表';
...
COMMENT ON COLUMN "public"."app_page"."access_level" IS E'访问级别（0=公开, 1=登录用户, 2=会员, 9=管理员）';
COMMENT ON COLUMN "public"."app_page"."is_enabled" IS E'是否启用';
COMMENT ON COLUMN "public"."app_page"."enable_platform" IS E'启用的平台列表';
...
COMMENT ON COLUMN "public"."check_in_plan"."is_enabled" IS E'是否启用。\n用于后台紧急停用计划，但保留配置与历史审计。';
COMMENT ON COLUMN "public"."check_in_plan"."timezone" IS E'计划展示时区。\n一期仍保留字段用于契约兼容与展示，但运行时自然日口径以部署时区为准。';
COMMENT ON COLUMN "public"."check_in_plan"."cycle_type" IS E'周期类型。\n一期只允许 `daily`、`weekly`、`monthly` 三类稳定值。';
COMMENT ON COLUMN "public"."check_in_plan"."cycle_anchor_date" IS E'周期锚点日期。\n使用 `date` 语义冻结计划切周期口径，不落成 timestamp。';
...
COMMENT ON COLUMN "public"."check_in_plan"."publish_start_at" IS E'发布时间窗开始时间。\n采用左闭右开口径中的左边界，`null` 表示不限制开始时间。';
COMMENT ON COLUMN "public"."check_in_plan"."publish_end_at" IS E'发布时间窗结束时间。\n采用左闭右开口径中的右边界，`null` 表示不限制结束时间。';
...
COMMENT ON COLUMN "public"."check_in_streak_reward_rule"."sort_order" IS E'排序值。\n仅影响后台展示和阈值列表排序，不参与唯一性判断。';
