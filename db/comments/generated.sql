...
COMMENT ON TABLE "public"."background_task" IS E'通用后台任务表。\n仅存储业务无关的任务生命周期、负载、进度、结果、错误和回滚诊断信息。';
COMMENT ON COLUMN "public"."background_task"."id" IS E'主键 ID。';
COMMENT ON COLUMN "public"."background_task"."task_id" IS E'对外暴露的任务 ID。';
COMMENT ON COLUMN "public"."background_task"."task_type" IS E'任务类型，由业务处理器注册。';
COMMENT ON COLUMN "public"."background_task"."display_name" IS E'面向运营展示的任务主体名称，如作品名；为空表示使用任务类型展示。';
COMMENT ON COLUMN "public"."background_task"."operator_type" IS E'操作者类型（1=后台管理员，2=系统）。';
COMMENT ON COLUMN "public"."background_task"."operator_user_id" IS E'后台管理员操作者 ID；系统任务为空。';
COMMENT ON COLUMN "public"."background_task"."status" IS E'当前任务状态（1=待处理，2=处理中，3=最终写入中，4=成功，5=失败，6=已取消，7=回滚失败）。';
COMMENT ON COLUMN "public"."background_task"."payload" IS E'原始任务负载。';
COMMENT ON COLUMN "public"."background_task"."progress" IS E'任务进度快照。';
COMMENT ON COLUMN "public"."background_task"."result" IS E'成功结果。';
COMMENT ON COLUMN "public"."background_task"."error" IS E'失败或取消错误信息。';
COMMENT ON COLUMN "public"."background_task"."residue" IS E'处理器记录的待回滚残留信息。';
COMMENT ON COLUMN "public"."background_task"."rollback_error" IS E'回滚失败诊断信息。';
COMMENT ON COLUMN "public"."background_task"."dedupe_key" IS E'活跃任务去重键；同一任务类型下 PENDING/PROCESSING/FINALIZING 期间唯一。';
COMMENT ON COLUMN "public"."background_task"."serial_key" IS E'执行期串行键；同一任务类型下 PROCESSING/FINALIZING 期间唯一。';
COMMENT ON COLUMN "public"."background_task"."retry_count" IS E'已重试次数。';
COMMENT ON COLUMN "public"."background_task"."max_retries" IS E'最大允许重试次数。';
COMMENT ON COLUMN "public"."background_task"."cancel_requested_at" IS E'取消请求时间，非空表示运行中任务需要协作取消。';
COMMENT ON COLUMN "public"."background_task"."claimed_by" IS E'当前 claim 的 worker 标识。';
COMMENT ON COLUMN "public"."background_task"."claim_expires_at" IS E'当前 claim 过期时间，仅用于 PROCESSING 回收，不用于 FINALIZING 重放。';
COMMENT ON COLUMN "public"."background_task"."started_at" IS E'开始处理时间。';
COMMENT ON COLUMN "public"."background_task"."finalizing_at" IS E'进入最终写入阶段时间。';
COMMENT ON COLUMN "public"."background_task"."finished_at" IS E'任务完成时间。';
COMMENT ON COLUMN "public"."background_task"."created_at" IS E'创建时间。';
COMMENT ON COLUMN "public"."background_task"."updated_at" IS E'更新时间。';
COMMENT ON TABLE "public"."background_task_conflict_key" IS E'后台任务业务冲突键表。\n记录业务层声明的活动 reservation，用于跨任务的同源、同名和同章节互斥。';
COMMENT ON COLUMN "public"."background_task_conflict_key"."id" IS E'主键 ID。';
COMMENT ON COLUMN "public"."background_task_conflict_key"."task_id" IS E'对外后台任务 ID。';
COMMENT ON COLUMN "public"."background_task_conflict_key"."task_type" IS E'任务类型。';
COMMENT ON COLUMN "public"."background_task_conflict_key"."conflict_key" IS E'业务冲突键。';
COMMENT ON COLUMN "public"."background_task_conflict_key"."released_at" IS E'释放时间；为空表示仍然占用。';
COMMENT ON COLUMN "public"."background_task_conflict_key"."created_at" IS E'创建时间。';
COMMENT ON COLUMN "public"."background_task_conflict_key"."updated_at" IS E'更新时间。';
...
COMMENT ON TABLE "public"."work_comic_archive_import_preview_session" IS E'漫画压缩包预解析会话表。\n在压缩包上传前先注册可取消会话，确保关闭弹窗时可以强一致清理预确认残留。';
COMMENT ON COLUMN "public"."work_comic_archive_import_preview_session"."id" IS E'主键ID。';
COMMENT ON COLUMN "public"."work_comic_archive_import_preview_session"."task_id" IS E'对外暴露的任务ID。';
COMMENT ON COLUMN "public"."work_comic_archive_import_preview_session"."work_id" IS E'关联作品ID。';
COMMENT ON COLUMN "public"."work_comic_archive_import_preview_session"."chapter_id" IS E'单章节导入目标章节ID。';
COMMENT ON COLUMN "public"."work_comic_archive_import_preview_session"."status" IS E'预解析会话状态（1=开放，2=丢弃中）。';
COMMENT ON COLUMN "public"."work_comic_archive_import_preview_session"."expires_at" IS E'会话过期时间。';
COMMENT ON COLUMN "public"."work_comic_archive_import_preview_session"."created_at" IS E'创建时间。';
COMMENT ON COLUMN "public"."work_comic_archive_import_preview_session"."updated_at" IS E'更新时间。';
