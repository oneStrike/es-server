...
export const userFavorite = snakeCase.table(
  ...
  (table) => [
     ...
     */
    unique('user_favorite_target_type_target_id_user_id_key').on(
      table.targetType,
      table.targetId,
      table.userId,
    ),
     ...
     * 目标类型与目标ID联合索引
     */
    index('user_favorite_target_type_target_id_idx').on(
      table.targetType,
      table.targetId,
    ),
    /**
    ...
    /**
     * 收藏目标类型闭集约束：1=漫画，2=小说，3=论坛主题
     */
    check(
      'user_favorite_target_type_valid_chk',
      sql`${table.targetType} in (1, 2, 3)`,
    ),
  ],
)
