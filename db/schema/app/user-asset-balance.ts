 ...
 */
export const userAssetBalance = pgTable('user_asset_balance', {
  /** 主键 ID。 */
  ...
  check(
    ...
    sql`btrim(${table.assetKey}) = '' or btrim(${table.assetKey}) <> ''`,
  ),
  check('user_asset_balance_balance_non_negative_chk', sql`${table.balance} >= 0`),
])

export type UserAssetBalance = typeof userAssetBalance.$inferSelect
export type UserAssetBalanceSelect = UserAssetBalance
