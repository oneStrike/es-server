 ...
 * 应用协议表 - 存储隐私政策、用户协议等
 */
export const appAgreement = snakeCase.table(
  'app_agreement',
  {
    /**
     * 主键ID
  ...
  (table) => [
     ...
     * 标题与发布状态索引
     */
    index('app_agreement_title_is_published_idx').on(
      table.title,
      table.isPublished,
    ),
  ],
 ...
 * 应用协议签署记录表
 */
export const appAgreementLog = snakeCase.table(
  'app_agreement_log',
  {
    /**
     * 主键ID (使用BigInt防止记录过多)
     ...
     * 签署时间
     */
    agreedAt: timestamp({ withTimezone: true, precision: 6 })
      .defaultNow()
      .notNull(),
    /**
     * 签署IP
  ...
  (table) => [
     ...
     * 用户与协议索引
     */
    index('app_agreement_log_user_id_agreement_id_idx').on(
      table.userId,
      table.agreementId,
    ),
    /**
...
export type AppAgreementSelect = typeof appAgreement.$inferSelect
export type AppAgreementInsert = typeof appAgreement.$inferInsert
export type AppAgreementLogSelect = typeof appAgreementLog.$inferSelect
