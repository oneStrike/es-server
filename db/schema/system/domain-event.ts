...
export const domainEvent = pgTable('domain_event', {
  ...
  index('domain_event_event_key_created_at_idx').on(table.eventKey, table.createdAt.desc()),
  index('domain_event_domain_occurred_at_idx').on(table.domain, table.occurredAt.desc()),
  index('domain_event_subject_type_subject_id_idx').on(table.subjectType, table.subjectId),
  index('domain_event_target_type_target_id_idx').on(table.targetType, table.targetId),
])
