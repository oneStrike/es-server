...
export const contentImportItem = snakeCase.table(
  ...
  (table) => [
    ...
    check(
      'content_import_item_item_type_valid_chk',
      sql`${table.itemType} in (1)`,
    ),
    check(
      'content_import_item_status_valid_chk',
      sql`${table.status} in (1, 2, 3, 4, 5, 6)`,
    ),
    check(
      'content_import_item_stage_valid_chk',
      sql`${table.stage} in (1, 2, 3, 4, 5, 6, 7, 8)`,
    ),
    check(
      'content_import_item_title_nonblank_chk',
      sql`length(trim(${table.title})) > 0`,
    ),
    check(
      'content_import_item_failure_count_non_negative_chk',
      sql`${table.failureCount} >= 0`,
    ),
    check(
      'content_import_item_auto_retry_count_non_negative_chk',
      sql`${table.autoRetryCount} >= 0`,
    ),
    check(
      'content_import_item_max_auto_retries_non_negative_chk',
      sql`${table.maxAutoRetries} >= 0`,
    ),
    check(
      'content_import_item_image_total_non_negative_chk',
      sql`${table.imageTotal} >= 0`,
    ),
    check(
      'content_import_item_image_success_count_non_negative_chk',
      sql`${table.imageSuccessCount} >= 0`,
    ),
    check(
      'content_import_item_current_attempt_no_positive_chk',
