...
export const contentImportPreviewItem = snakeCase.table(
  ...
  (table) => [
    ...
    check(
      'content_import_preview_item_item_type_valid_chk',
      sql`${table.itemType} in (1)`,
    ),
    check(
      'content_import_preview_item_status_valid_chk',
      sql`${table.status} in (1, 2, 3)`,
    ),
    check(
      'content_import_preview_item_title_nonblank_chk',
      sql`length(trim(${table.title})) > 0`,
    ),
    check(
      'content_import_preview_item_image_total_non_negative_chk',
