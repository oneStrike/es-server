 ...
 */
export const forumUserActionLog = snakeCase.table(
  ...
  (table) => [
     ...
     * 目标类型与目标ID索引
     */
    index('forum_user_action_log_target_type_target_id_idx').on(
      table.targetType,
      table.targetId,
    ),
    /**
     ...
     * 用户与创建时间索引
     */
    index('forum_user_action_log_user_id_created_at_idx').on(
      table.userId,
      table.createdAt,
    ),
  ],
