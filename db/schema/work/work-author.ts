 ...
 * 作者信息模型
 */
export const workAuthor = snakeCase.table(
  'work_author',
  {
    /**
     * 主键ID
  ...
  (table) => [
     ...
     * 启用与推荐索引
     */
    index('work_author_is_enabled_is_recommended_idx').on(
      table.isEnabled,
      table.isRecommended,
    ),
    /**
     * 启用与删除时间索引
     */
    index('work_author_is_enabled_deleted_at_idx').on(
      table.isEnabled,
      table.deletedAt,
    ),
    /**
     ...
     */
    check(
      'work_author_gender_valid_chk',
      sql`${table.gender} in (0, 1, 2, 3, 4)`,
    ),
    /**
     * 作者类型闭合值域约束
     */
    check(
      'work_author_type_valid_chk',
      sql`${table.type} is null or ${table.type} <@ '{1,2}'::smallint[]`,
    ),
    /**
     * 推荐与作品数索引
     */
    index('work_author_is_recommended_work_count_idx').on(
      table.isRecommended,
      table.workCount.desc(),
    ),
    /**
     * 创建时间索引
     */
