...
export const workflowJob = snakeCase.table(
  ...
  (table) => [
    ...
    check(
      'workflow_job_workflow_type_nonblank_chk',
      sql`length(trim(${table.workflowType})) > 0`,
    ),
    check(
      'workflow_job_display_name_nonblank_chk',
      sql`length(trim(${table.displayName})) > 0`,
    ),
    check(
      'workflow_job_operator_type_valid_chk',
    ...
    check(
      'workflow_job_status_valid_chk',
      sql`${table.status} in (1, 2, 3, 4, 5, 6, 7, 8)`,
    ),
    check(
      'workflow_job_progress_percent_range_chk',
      sql`${table.progressPercent} between 0 and 100`,
    ),
    check(
      'workflow_job_selected_item_count_non_negative_chk',
      sql`${table.selectedItemCount} >= 0`,
    ),
    check(
      'workflow_job_success_item_count_non_negative_chk',
      sql`${table.successItemCount} >= 0`,
    ),
    check(
      'workflow_job_failed_item_count_non_negative_chk',
      sql`${table.failedItemCount} >= 0`,
    ),
    check(
      'workflow_job_skipped_item_count_non_negative_chk',
