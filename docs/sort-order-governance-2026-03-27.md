...
- 对于没有 `sortOrder` 的表，统一回退默认顺序是 `id desc`。
- `admin` 端仍可支持手动切换到 `createdAt desc, id desc` 或 `updatedAt desc, id desc` 以便操作。
...
- `sortOrder asc` 必须变为 `sortOrder asc, id asc`
- 仓库统一回退默认必须是 `id desc`
- `createdAt desc` 必须变为 `createdAt desc, id desc`
- `updatedAt desc` 必须变为 `updatedAt desc, id desc`

`buildDrizzlePageQuery()` 已经在分页辅助工具中为非唯一 `orderBy` 追加了 `id`。原始的 `db.query.*` 和手动 `select().orderBy(...)` 路径必须显式执行此操作。
...
- 不要添加或推断 `sortOrder`
- 仓库统一回退默认：`id desc`
- 如果业务需要时间、发布、活跃度、优先级或相关性排序，必须显式声明，并在非唯一字段后追加 `id`
...
| `content/work author relations` | `work_author_relation` | 受管理的关系/成员排序 | 是 | `workId` 范围内 `sortOrder asc, id asc` | 写入路径已按输入顺序分配顺序。 |
| `content/work category relations` | `work_category_relation` | 受管理的关系/成员排序 | 是 | `workId` 范围内 `sortOrder asc, id asc` | 写入路径当前写入反向编号；应规范化为一种约定。 |
| `content/work tag relations` | `work_tag_relation` | 受管理的关系/成员排序（候选） | 有条件是 | `workId` 范围内 `sortOrder asc, id asc` | 字段存在，但当前写入路径未维护它。完全采用前必须修复。 |
...
| `config/dictionary item` | `sys_dictionary_item` | 受管理的展示资源 | 是 | `dictionaryCode` 范围内 `sortOrder asc, id asc` | App 读取路径已使用 `sortOrder asc`；admin 页面应对齐。 |
...
| `forum/section` | `forum_section` | 受管理的展示资源 | 是 | `groupId` 范围内 `sortOrder asc, id asc` | App 端已将版块视为排列资源；admin 页面应默认使用相同契约。 |
...
| `interaction/emoji asset` | `emoji_asset` | 受管理的展示资源 | 是 | `packId` 范围内 `sortOrder asc, id asc` | 当前 admin/app 行为已基本对齐。 |
...
- 定义 `sortOrder` 方向 (`asc`)
- 定义稳定排序表达式的辅助构建器
- 定义无 `sortOrder` 资源的统一回退默认 (`id desc`)
- 定义默认追加/新项目策略
- 公开一小套可复用的辅助工具
...
```ts
export type ManagedOrderMode = 'sortOrder' | 'id' | 'createdAt' | 'updatedAt'

export function buildManagedSortOrder(
  mode: ManagedOrderMode = 'sortOrder',
): Array<Record<string, 'asc' | 'desc'>>

export function buildScopedSortOrderWhere(
  scopeField: string,
...
- `sortOrder` 资源页面辅助工具应返回：
  ...
  - 手动创建视图：`[{ createdAt: 'desc' }, { id: 'desc' }]`
  - 手动更新视图：`[{ updatedAt: 'desc' }, { id: 'desc' }]`
...
- 稳定排序构建器返回 `sortOrder + id`
- 无 `sortOrder` 的统一回退构建器返回 `id desc`
- 时间排序构建器返回 `createdAt/updatedAt + id`
- 不支持的字段抛出显式错误

### 10.2 服务测试

- 受管理的资源页面默认使用 `sortOrder asc, id asc`
...
- `admin` 和 `app` 默认使用相同的 `sortOrder asc, id asc`
- `admin` 可手动切换到 `createdAt/updatedAt`
- 如果模块没有 `sortOrder`，仓库统一回退默认是 `id desc`
