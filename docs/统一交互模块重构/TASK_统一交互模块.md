...
graph TB
    ...
    subgraph "Phase 2: 基础设施"
        T1[T1: 创建 interaction 模块结构]
        T3[T3: 创建常量和类型定义]
        T4[T4: 创建基础 DTO]
    end

    subgraph "Phase 3: 核心服务"
        T5[T5: 创建目标校验器]
        T6[T6: 创建计数处理器]
        T7[T7: 创建交互基类服务]
        T8[T8: 创建点赞服务]
    ...
    subgraph "Phase 4: 模块集成"
        T15[T15: 创建 InteractionModule]
        T17[T17: 更新 ContentModule 集成]
        T18[T18: 更新 ForumModule 集成]
        T19[T19: 删除旧表和旧服务]
    end

    subgraph "Phase 5: 验证测试"
        T23[T23: 验证功能完整性]
    end
