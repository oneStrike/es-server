...
- 本轮只有一个主任务，不建议再拆成多份语义碎片文档。
- 若实施时需要拆分代码工作，可以在代码层并行，但文档层继续保持单任务闭环。

## 9. 变更入口

- 若任务边界变化，优先修改 [06-task-state-flow-and-audit-correction.md](./p0/06-task-state-flow-and-audit-correction.md)
- 若优先级、依赖、波次或状态变化，只修改本文
- 若验收标准或证据位变化，只修改 [final-acceptance-checklist.md](./checklists/final-acceptance-checklist.md)
