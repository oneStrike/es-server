...
- 属地字段命名和返回口径以 [README.md](/E:/Code/es/es-server/docs/audits/ip-geolocation-work-items/README.md) 为准，不再派生第二套命名。
...
- app 端评论我的列表 / 回复列表 / 目标评论列表返回字段检查
- 登录态 token 落库样例检查
- 请求审计日志、论坛操作日志落库样例检查
- ForumTopic 公开列表 / 详情 / 我的主题组装测试需要补齐 `geo*` 字段断言。
- Comment 列表 / 回复 / 目标评论测试需要补齐 `geo*` 字段断言。
...
- 评论列表返回属地字段会波及多个 DTO 和查询投影，容易出现“schema 已加字段但接口漏返回”的契约漂移。
