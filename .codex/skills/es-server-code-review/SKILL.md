...
- 始终对每个可疑实现、每条 finding 做“规则 × 代码面”交叉映射；不要因为先命中一条规则、一个症状或一个高优问题就停止扩展。
...
- 对每个可疑点 / finding，显式确认其已完成“适用规则映射”和“相关代码面映射”；只有完成交叉映射后，才允许把该点位视为闭合。
...
4. 对每个可疑点 / finding 执行交叉映射：
   - 回看全部适用规则文档，而不是只回看第一个命中的那一条规则。
   - 回看相关代码面：上下游调用点、DTO、type、schema、migration、tests。
   - 确认是否同时存在第二性质问题，例如“真实缺陷 + 规范违例”、“DTO 漂移 + schema 脱锚”、“注释问题 + 类型重复定义”。
   - 若同一实现同时命中多条规则，或同时属于规范问题与缺陷 / 风险问题，分别记录，不得因为已有一条 finding 就吞掉另一条。
5. 对每条发现的问题，补齐行号、影响和修复方向。
6. 对尚未完成的矩阵项继续推进，直到全部完成；发现 P0 / P1 / P2 都不是停止条件。
...
- 若同一处同时属于规范问题和缺陷问题，两边都要检查是否成立；可以按主要性质归类，但不能因为先报了一边就停止另一边的规则映射。
...
- 不要因为某个症状已经解释得通，就停止对同一实现做其余适用规则和相关代码面的交叉检查。
