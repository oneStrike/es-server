...
- **兼容优先演进**：默认维持单一正式入口；若改动会影响已有客户端或引入 breaking change，必须显式设计版本化或兼容窗口，不能静默替换对外契约。
...
- 不把历史路径无限期保留为正式入口；必要的兼容入口必须有明确适用范围、弃用说明与下线计划。
...
- 默认只保留一套正式入口；如果路由、入参、响应结构、分页语义或错误语义会影响现有客户端，必须先评估兼容方案。
- 兼容方案优先顺序：
  1. 使用 Nest versioning 显式并行承载旧版本与新版本。
  2. 在当前 controller 下保留短期兼容路由或兼容 controller，并明确标注适用客户端与下线条件。
  3. 若无存量客户端依赖，直接收敛到新入口，不保留历史壳层。
- 兼容入口必须复用同一领域 service 或共享编排逻辑，避免为兼容而复制第二套业务实现。
- 兼容入口不是新能力的默认承载层；新增功能应优先落在当前正式契约上。
...
- 对纯副作用、幂等开关、批量状态变更这类“客户端只关心是否成功”的接口，优先返回 `boolean` 或使用 `204 No Content`，避免包装无意义成功对象。
- `create`、需要前端立刻消费新状态的 `update`、需要回显新快照的动作型接口，可以返回 `id`、最小成功载荷或资源快照，不强制收敛为 `boolean`。
- Service 返回值按领域需求建模；只有当 controller 侧确实只需要成功信号时，才在应用边界收敛为 `boolean`。
...
- 默认只保留一套正式入口；`compat`、`legacy`、版本化 controller 仅在保护存量客户端时临时存在，并需附带范围说明与下线计划。
- 设计文档、Swagger、实际代码必须同步更新。
- 新增接口前先检查是否已存在同语义路径或同职责 controller。

## 10. 实现检查顺序

1. 先确认资源是否已有正式入口；若本次变更会影响已有契约，先判断是否需要版本化或兼容窗口。
2. 确认接口属于分页、列表、详情、统计还是业务动作。
3. 选择对应的标准路径与 tag。
4. 确认输入 DTO、输出 DTO、Swagger 文档匹配。
5. 确认 admin 接口权限与审计装饰器正确。
6. 对增删改接口确认返回语义是否合理：纯成功确认场景优先 `boolean/204`，需要回显新状态时返回最小必要载荷，并检查 Service 与数据库写操作未引入无意义 `returning()`。
7. 涉及已有客户端时，复核兼容入口、版本号、弃用说明与测试范围。
8. 变更后执行类型检查，并复扫路径语义是否一致。
...
- [ ] 涉及 breaking change 或存量客户端的接口已评估版本化/兼容窗口，没有静默替换旧契约。
- [ ] 纯成功确认类接口优先使用 `boolean/204`；需要回显新状态时返回最小必要载荷，没有被规则反向压成无意义 `boolean`。
