...
- 与 DTO 同构的 `Input/View` 类型不重复定义；如需语义名，仅允许类型别名。
- `*.type.ts` 仅承载内部领域结构（非 HTTP 契约）。
...
- 新增业务内部类型文件默认使用 `*.type.ts`；历史 `*.types.ts` 不要求批量迁移。

## 3. 命名规范

- 数据库读模型：`Xxx`；插入模型：`XxxInsert`。
- 内部领域类型：`XxxContext`、`XxxPayload`、`XxxSnapshot`、`XxxAggregation`、`XxxRow`、`XxxResult`。
- DTO 同构别名（可选）：`type QueryXxxInput = QueryXxxDto`、`type XxxView = XxxResponseDto`。
...
- 仅当结构不属于 HTTP 契约时，才在 `*.type.ts` 新增对象类型。
...
- [ ] 未新增与 DTO 同构的重复 `Input/View` 类型（或已收敛为别名）。
- [ ] 新增内部业务类型优先使用 `*.type.ts`。
