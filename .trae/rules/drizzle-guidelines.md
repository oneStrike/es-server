...
3. 所有写操作必须包裹 `drizzle.withErrorHandling(() => ...)`，或使用等价的 `withTransaction` 统一封装。
4. update/delete 在语义上必须保证资源存在时，必须调用 `drizzle.assertAffectedRows(result, '资源不存在')`；幂等删除等允许 0 行变更的场景除外。
5. 分页统一使用 `drizzle.ext.findPagination(...)`；业务代码不得自行计算 `offset/limit` 或单独定义分页返回结构。
...
9. 事务统一使用 `db.transaction(async (tx) => ...)` 或 `drizzle.withTransaction(...)`，并在调用链中显式传递 tx；禁止 `tx: any` 与忽略传入事务。
...
4. 复杂列表查询优先采用“主表分页 + 关系补充”模式，避免深层联表嵌套。
...
2. 不要绕过 `withErrorHandling` 与 `assertAffectedRows`。
...
2. 写操作统一经过 `withErrorHandling`；必须保证资源存在的更新/删除使用了 `assertAffectedRows`。
3. 分页统一走 `ext.findPagination`，未在业务层重复实现分页语义。
4. 事务类型无 `any`，且没有忽略传入事务的写法。
5. 原生 SQL 已下沉到业务模块内 helper/私有方法，业务层没有手工 `rows` 解包。
6. 代码中未新增 `db._query`，且不存在将用户输入传给 `sql.raw()` 的写法。
7. 类型检查通过，并且相关模块的主要查询/写入路径已完成自测。
