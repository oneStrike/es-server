...
- 表名前缀、后缀、关系表与 `app_user*` / `user_*` 边界，统一遵循 `./TABLE_NAMING_SPEC.md`。
...
- 常规 schema 差异默认通过 `pnpm db:generate` 生成；新增表、字段、约束、索引等可由工具准确表达的变更，不要绕过生成流程手写 migration。
- 如果 `pnpm db:generate` 过程中出现任何交互式确认、重命名选择或冲突选择，必须立即停止任务，并由用户亲自在终端执行该命令并完成交互。
- 若 rename、backfill、数据修复、分步安全发布、锁风险控制、并发索引或工具暂不支持的 DDL 无法由生成结果准确表达，允许在评审后手动补充或调整 migration SQL，但必须同时满足：
  - 优先先拿到 `pnpm db:generate` 的基线结果，或在交付说明中写明为何当前变更无法生成基线。
  - 手写部分只覆盖必要语句，不把本可自动生成的常规变更整体改写成手工 migration。
  - 在交付说明、任务单或迁移注释中记录变更目的、风险点以及执行顺序。
  - 同步复核 schema、应用代码、`db/comments/generated.sql` 与迁移语义的一致性。
- 禁止为了省事使用空白 migration 骨架或自定义 SQL 去替代本可直接由 `pnpm db:generate` 处理的常规变更。
...
- [ ] 常规迁移已优先通过 `pnpm db:generate` 生成；若出现交互，已由用户亲自执行并完成。
- [ ] 若 migration 存在手写补充，已记录原因、范围与执行风险，且未替代本可自动生成的常规变更。
