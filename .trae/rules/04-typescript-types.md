...
- 可复用的业务类型默认放在 owner 域的 `*.type.ts` 中；不要为了图省事散落在多个 service / controller 文件里。
- 纯 TypeScript 的 `type` / `interface` 定义统一放在 `*.type.ts` 中；不要在 service / controller / module / dto / constant 文件里新增顶层类型定义。
- 方法 / 函数签名中的复杂类型表达式也必须先在 `*.type.ts` 中命名后再引用；不要在入参、返回值或泛型参数位置直接写内联对象类型、`Pick`、`Omit`、`Partial`、联合 / 交叉类型、函数类型或嵌套泛型。
- `*.type.ts` 只承载非 HTTP 的内部领域结构、查询上下文、聚合结果、事件 payload、适配器入参等类型。
