...
describe('auto deploy governance regressions', () => {
  it('uses fetch + reset instead of stash + pull in the remote deploy script', () => {
    expect(remoteDeployScript).toMatch(
      /reset --hard "origin\/\$\{CURRENT_BRANCH\}"/,
    )
    ...
    expect(remoteDeployScript).not.toContain(
      'git_with_retry pull origin "${CURRENT_BRANCH}"',
    )
  })

  it('deploys server before both frontends by default', () => {
    expect(remoteDeployScript).toContain(
      'PROJECTS=("es-server" "es-admin" "es-app-v2")',
    )
    expect(localDeployScript).toContain(
      'for project in es-server es-admin es-app-v2; do',
    )
  })

  it('always refreshes frontend proxies after server deploy to avoid stale upstreams', () => {
    expect(remoteDeployScript).toContain(
    ...
    expect(remoteDeployScript).not.toContain('PENDING_PROJECTS')
    expect(localDeployScript).toContain(
      'refresh_frontend_proxies_after_server_deploy',
    )
    expect(localDeployScript).not.toContain(
      'should_refresh_frontend_after_server_deploy',
    )
  })
})
