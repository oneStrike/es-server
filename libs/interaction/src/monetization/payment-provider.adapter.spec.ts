...
const PAYMENT_VERIFY_SECRET_ENV = 'MONETIZATION_TEST_PAYMENT_SECRET'
...
describe('Payment provider adapters', () => {
  ...
  function signPaymentFields(fields: Record<string, string | number>) {
  ...
  it('rejects unsigned payment notify payloads', () => {
    const adapter = new AlipayPaymentProviderAdapter()
  ...
  it('verifies signed payment notify payloads against order, amount, and channel', () => {
    const adapter = new AlipayPaymentProviderAdapter()
    const config = {
      ...baseConfig,
      configMetadata: {
    ...
    expect(
      adapter.verifyNotify({
        order: baseOrder,
        config,
        payload: buildSignedPaymentPayload(),
      }),
    ).toBe(true)
  ...
  it('requires the auto-renew agreement number to be part of the signed notify payload', () => {
    const adapter = new AlipayPaymentProviderAdapter()
    const order = {
      ...baseOrder,
