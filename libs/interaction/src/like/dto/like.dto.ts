...
export class LikeStatusQueryDto {
  @ApiProperty({ description: '目标类型', enum: InteractionTargetType, example: 1 })
  @IsInt()
  @IsNotEmpty()
  targetType!: InteractionTargetType

  @ApiProperty({ description: '目标ID数组', example: [1, 2, 3], type: [Number] })
  @IsInt({ each: true })
  @IsNotEmpty()
  targetIds!: number[]
}
...
export class LikeListQueryDto {
  @ApiPropertyOptional({ description: '目标类型筛选', enum: InteractionTargetType, example: 1 })
  @IsInt()
  @IsOptional()
  targetType?: InteractionTargetType
