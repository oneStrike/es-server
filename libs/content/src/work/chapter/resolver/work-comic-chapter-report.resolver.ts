...
import { IReportTargetResolver } from '@libs/interaction/report/interfaces/report-target-resolver.type'
  ...
  implements IReportTargetResolver, OnModuleInit
{
  ...
  // 初始化 WorkComicChapterReportResolver 依赖。
  constructor(private readonly reportService: ReportService) {}

  // 模块初始化时注册解析器到举报服务，使举报服务能够识别并处理漫画章节类型的举报请求。
  onModuleInit() {
    this.reportService.registerResolver(this)
  }

  // 解析目标漫画章节的场景元数据，验证章节存在性并返回场景类型和场景ID。
