...
export class ThirdPartyResourceThrottleService {
  private readonly channels = new Map<string, ThirdPartyResourceThrottleState>()

  // 注入系统配置读取器，运行时始终读取已归一化的三方资源解析配置。
  constructor(private readonly configReader: ConfigReader) {}

  // 等待三方 provider API/discovery 通道可用。
  async waitForApiSlot(channel: string) {
    await this.waitForChannel(channel, 'api')
  }

  // 等待三方远程图片下载通道可用。
  async waitForImageSlot(channel: string) {
    await this.waitForChannel(channel, 'image')
  }

  // 返回三方 provider host discovery 缓存 TTL，供 HTTP client 控制缓存失效。
  getHostCacheTtlMs() {
    return (
  ...
  private async waitForChannel(
    channel: string,
    kind: ThirdPartyResourceThrottleChannelKind,
  ) {
    ...
    state.queueSize += 1
    const intervalMs =
      kind === 'api' ? config.apiIntervalMs : config.imageIntervalMs
  ...
  // 为每个 provider 声明的节流通道单独维护排队状态。
  private getChannelState(channel: string) {
    if (channel.length === 0) {
      throw new BusinessException(
        BusinessErrorCode.OPERATION_NOT_ALLOWED,
        '三方资源解析节流通道缺失',
