import type { BaseUserAssetsSummaryDto } from './dto/user-assets.dto'
import type {
  UserAssetsCountRow,
...
import { CouponInstanceStatusEnum } from '@libs/interaction/coupon/coupon.constant'
import { DOWNLOAD_WORK_CHAPTER_TARGET_TYPES } from '@libs/interaction/download/download.constant'
import { MembershipSubscriptionStatusEnum } from '@libs/interaction/membership/membership.constant'
import { READING_COIN_ASSET_KEY } from '@libs/interaction/wallet/wallet.constant'
...
export class UserAssetsService {
  ...
  /**
   * 聚合当前用户的钱包、会员、优惠券、购买、下载和互动计数。
   *
   * 查询并行执行且只读；字段和值域维持既有 HTTP 与 Swagger 合同。
   */
  async getUserAssetsSummary(
    userId: number,
  ): Promise<BaseUserAssetsSummaryDto> {
    const [
      commentCount,
      likeCount,
      favoriteCount,
