...
export class ForumModeratorApplicationService {
  ...
  // 将权限值映射为可读文案。 仅用于详情与列表响应，不参与审核通过时的权限裁剪判断。
  ...
  // 校验申请用户存在且未软删除。 在创建申请前提前失败，避免写路径产生指向无效用户的申请单。
  ...
  // 校验申请板块存在且仍处于启用状态。 禁止用户对已删除或已禁用板块继续发起版主申请。
  ...
  // 校验申请用户当前不是有效版主。 审核通过和创建申请都会复用该约束，避免重复授予版主身份。
  ...
  // 将 0 行更新统一收口为稳定业务异常。 用于申请审核这类先读后写链路，避免并发下静默失败。
  ...
  // 查询后台版主申请详情。 仅返回未软删除记录，缺失时抛出稳定的 not-found 异常。
  ...
  // 查询当前用户的版主申请详情。 通过 applicantId 绑定当前用户，避免越权读取他人申请单。
  ...
  // 提交版主申请。 对同一用户同一板块的历史申请会复用原记录并重置为待审核状态，避免重复插入脏数据。
  ...
  // 分页查询版主申请。 nickname 过滤会先解析用户集合，再收敛到申请表，保证申请列表与用户昵称搜索口径一致。
  ...
  // 分页查询当前用户的申请记录。 在通用分页查询基础上强制追加 applicantId 约束，避免出现双份实现。
  ...
  // 审核版主申请。 审核通过时会先校验板块与用户状态，再调用版主服务授予板块版主身份，最后落审核结果。
  ...
  // 后台软删除版主申请。 仅对未软删除记录生效，并通过受影响行数统一抛出缺失异常。
  ...
  // 用户删除自己的版主申请。 删除条件会同时绑定 applicantId，防止用户删除他人申请单。
