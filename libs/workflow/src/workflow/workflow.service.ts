...
class WorkflowSnapshotDriftError extends Error {}
...
export class WorkflowService {
  ...
  // 确认草稿并创建首次 attempt；事务外发现锁资源，锁后重读再执行状态迁移。
  async confirmDraft(input: WorkflowJobIdDto) {
    for (let attempt = 0; attempt < 2; attempt += 1) {
      const discoveredJob = await this.readJobWithDb(input.jobId, this.db)
        ...
        return await this.drizzle.withTransaction({
          execute: async (tx) => {
            await acquireIntegrityLocks(tx, [
              exclusiveIntegrityLock(
                tableIntegrityLock('workflow_job', discoveredJob.id),
              ),
            ])
            const lockedJob = await this.readJobWithDb(input.jobId, tx)
            if (lockedJob.id !== discoveredJob.id) {
              throw new WorkflowSnapshotDriftError()
            }
            return this.applyConfirmDraft(lockedJob, tx)
          },
        })
      } catch (error) {
        if (!(error instanceof WorkflowSnapshotDriftError)) {
          throw error
        }
        if (attempt === 1) {
  ...
  // 在调用方已完成完整锁集合获取和权威重读后确认草稿；apply 阶段禁止再次取锁。
  async applyConfirmDraft(
    job: Pick<
      WorkflowJobSelect,
      'id' | 'jobId' | 'selectedItemCount' | 'status'
    >,
    tx: DbTransaction,
  ) {
  ...
  private async createAttemptWithDb(
    job: Pick<WorkflowJobSelect, 'id' | 'selectedItemCount'>,
