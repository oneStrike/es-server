...
} from '@libs/eventing/eventing/eventing.constant'
import { getMessageConsumerEventLabel } from '@libs/message/eventing/message-consumer-event.constant'
...
export class MessageMonitorService {
  ...
  async retryNotificationDelivery(
    ...
    const deliveryId = this.parsePositiveIntegerId(
      body.deliveryId,
      'deliveryId',
    )
    ...
    try {
      const result = await this.domainEventDispatchService.retryFailedDispatch(
        delivery.dispatchId,
  ...
  async getWsMonitorSummary(query: QueryMessageWsMonitorDto) {
    ...
    const aggregateRows = await this.db
      .select({
        requestCount:
          sql<number>`COALESCE(SUM(${messageWsMetric.requestCount}), 0)::int`.mapWith(
            Number,
          ),
        ackSuccessCount:
          sql<number>`COALESCE(SUM(${messageWsMetric.ackSuccessCount}), 0)::int`.mapWith(
            Number,
          ),
        ackErrorCount:
          sql<number>`COALESCE(SUM(${messageWsMetric.ackErrorCount}), 0)::int`.mapWith(
            Number,
          ),
        ackLatencyTotalMs:
          sql<bigint>`COALESCE(SUM(${messageWsMetric.ackLatencyTotalMs}), 0)::bigint`.mapWith(
            BigInt,
          ),
        reconnectCount:
          sql<number>`COALESCE(SUM(${messageWsMetric.reconnectCount}), 0)::int`.mapWith(
            Number,
          ),
        resyncTriggerCount:
          sql<number>`COALESCE(SUM(${messageWsMetric.resyncTriggerCount}), 0)::int`.mapWith(
            Number,
          ),
        resyncSuccessCount:
          sql<number>`COALESCE(SUM(${messageWsMetric.resyncSuccessCount}), 0)::int`.mapWith(
            Number,
          ),
        fanoutSkippedCount:
          sql<number>`COALESCE(SUM(${messageWsMetric.fanoutSkippedCount}), 0)::int`.mapWith(
            Number,
          ),
        fanoutPublishErrorCount:
          sql<number>`COALESCE(SUM(${messageWsMetric.fanoutPublishErrorCount}), 0)::int`.mapWith(
            Number,
          ),
      })
