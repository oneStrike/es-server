...
import { ChatRealtimeEventConsumer } from '../chat/chat-realtime-event.consumer'
...
/**
 * 消息 consumer dispatch worker。
 * 当前显式处理 notification 与 chat_realtime 两类 consumer。
 */
@Injectable()
export class MessageEventDispatchWorker {
  constructor(
    ...
    for (const item of claimedDispatches) {
      try {
        if (item.dispatch.consumer === DomainEventConsumerEnum.NOTIFICATION) {
          await this.notificationEventConsumer.consume(
            item.event,
            item.dispatch,
          )
        } else if (
          ...
          await this.chatRealtimeEventConsumer.consume(
            item.event,
            item.dispatch,
          )
        }
