...
export abstract class BaseRequestLogInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    return next.handle().pipe(
      tap(() => {
        // 请求成功时记录日志
        void this.logRequest(request, response, startTime, null)
      }),
      catchError((error) => {
        // 请求失败时记录日志
        void this.logRequest(request, response, startTime, error)
    ...
    try {
      ...
      setImmediate(() => {
        this.requestLogService
          .createRequestLog(createRequestLogDto)
          .catch((error) => {
            this.logger.error(
              `${this.getLogPrefix()}异步记录请求日志失败: ${error.message}`,
              error.stack,
            )
          })
      })
